...
def main() -> None:
    ...
    root = tk.Tk()
    root.title("Blind Pathfinding V2.0")
    ...
    start_messages = ["", "Welcome to the Python Blind Pathfinder v2.",
                      "\n",
                      ...
                      "Differences to v1:",
                      " - using a new reverse search algorithm to find a cleaner path.",
                      " - cleaned up a lot of code, using more functions rather than repetitions."]
...
def pathfinding(event_enter):
    ...
    while(game_state == 3):
        ...
        possible_squares = find_possible_squares(current_square, False, path)
        ...
        best_square_coordinates = find_best_square(possible_squares, False)

        # clear old possible square list
        possible_squares.clear()
        
        print(f"Best square: {best_square_coordinates}.")
        ...
        if(current_square == session._grid._end_square_coordinates):
            ...
            print("Successfully found unsanitised path.")

            # time to sanitise the path
            
            """
            # first reverse the found path
            reversed_path = path[::-1]
            """
            
            # remove the picked setting from the squares
            # except for the end square
            for square in path:
                if(square == session._grid._end_square_coordinates):
                    pass
                else:
                    session._grid.get_grid_square(square).set_is_picked(False)
                print(session._grid.get_grid_square(square).get_is_picked())

            # new path
            new_path = []

            # we redraw a new path from the end until the
            # start only using the previous path's
            # squares
            while(current_square != session._grid._start_square_coordinates):
                # get new possible squares from current square
                possible_squares = find_possible_squares(current_square, True, path)

                # if no possible squares, then the pathfinder has failed
                # THIS SHOULD NEVER HAPPEN ON THE REVERSE
                ...
                # in order of highest to lowest priority:
                # 1. start square
                # 2. square with the lowest vector hypotenuse length to the start square

                best_square_coordinates = find_best_square(possible_squares, True)
                ...
                current_square = best_square_coordinates
                new_path.append(current_square)
                session._grid.get_grid_square(current_square).set_is_picked(True)
...
def find_possible_squares(current_square: (int, int), reversed_pathfinding: bool, path: [(int, int)]) -> [(int, int)]:
    """
    Find the available adjacent squares.
    """
    possible_squares = []
    ...
    for x_offset in range(-1, 2):
            for y_offset in range(-1, 2):
                ...
                if(x_offset == 0 and y_offset == 0):
                    ...
                    pass
                elif(possible_square_coordinates not in path and reversed_pathfinding):
                    # if not a part of the original path,
                    # then ignore
                    # this is for the sanitised path only
                    pass
                else:
...
def find_best_square(possible_squares: list, reversed_pathfinding: bool) -> (int, int):
    """
    Finds the best square from available adjacedcent
    squares based on the shortest vector hypotenuse.
    """

    # are we going from start to end (false) or
    # from end to start (true)
    if(reversed_pathfinding == True):
        goal = session._grid._start_square_coordinates
    else:
        goal = session._grid._end_square_coordinates
    ...
    for square_coordinates in possible_squares:
        ...
        vector_hypotenuse = math.sqrt(abs(square_coordinates[0] - goal[0])**2 + abs(square_coordinates[1] - goal[1])**2)
    ...
    return best_square_coordinates
